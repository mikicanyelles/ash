...
# NOTE: This function assumes that all helper functions (print_line_with_mainheader,
# ashexit, check_charge_mult, frange, read_surfacedict_from_file, 
# write_surfacedict_to_file, set_constraints, print_time_rel) and classes (ash, 
# NumGradclass, GeomeTRICOptimizer, GeomeTRICOptimizerClass, DLFIND_optimizerClass, 
# ASH_Results) are available in the scope where this function is called.

def calc_surface(fragment=None, theory=None, charge=None, mult=None, scantype='UNRELAXED', resultfile='surface_results.txt',
                 keepoutputfiles=True, keepmofiles=False,runmode='serial', 
                 optimizer='geomeTRIC', # <-- NEW: 'geomeTRIC' or 'dlfind'
                 coordsystem='dlc', # geomeTRIC only
                 maxiter=250, NumGrad=False,
                 extraconstraints=None, convergence_setting=None, conv_criteria=None, subfrctor=1, # geomeTRIC only
                 dlfind_residues=None, dlfind_jobtype='opt',# DL-FIND only
                 numcores=1, ActiveRegion=False, actatoms=None, RC1_range=None, RC1_type=None, RC1_indices=None,
                 RC2_range=None, RC2_type=None, RC2_indices=None):
    """Calculate 1D/2D surface
    ...
    Args:
        ...
        optimizer (str, optional): Optimizer to use: 'geomeTRIC' or 'dlfind'. Defaults to 'geomeTRIC'.
        
        # geomeTRIC options
        coordsystem (str, optional): Coordinate system for geomeTRICOptimizer. Defaults to 'dlc'.
        convergence_setting (str, optional): Convergence setting for geomeTRICOptimizer. Defaults to None.
        conv_criteria (dict, optional): Convergence criteria for geomeTRIC. Defaults to None.
        subfrctor (float, optional): Trust radius for geomeTRIC. Defaults to 1.
        
        # DL-FIND options
        dlfind_residues (list, optional): List of lists for DL-FIND HDLC residues. Defaults to None.

        maxiter (int, optional): Max number of Opt iterations (used by both optimizers). Defaults to 250.
        extraconstraints (dict, optional): Dictionary of additional constraints. Defaults to None.
        ActiveRegion (bool,optional): To use activeregion or not in optimization
        actatoms (list,optional): List of active atoms
        
        RC1_range (list): [start, stop, step] for Reaction Coordinate 1.
        RC1_type (str): Constraint type for RC1 (e.g., 'bond', 'angle', 'bond_diff').
        RC1_indices (list): Atom indices for RC1, e.g., [[0, 1]] or [[0, 1, 2, 3]].
        RC2_range (list, optional): [start, stop, step] for RC2.
        RC2_type (str, optional): Constraint type for RC2.
        RC2_indices (list, optional): Atom indices for RC2.

    Returns:
        ASH_Results: An ASH_Results object containing the surface points.
    """
    module_init_time=time.time()
    print_line_with_mainheader("CALC_SURFACE FUNCTION")
    print(f"Selected optimizer: {optimizer.upper()}")
    ...
    #Checking if list of lists. If so then we apply multiple constraints for this reaction coordinate (e.g. symmetric bonds)
    #Here making list of list in case only a single list was provided
    if not any(isinstance(el, list) for el in RC1_indices):
        RC1_indices=[RC1_indices]
    #2D SCAN
    if dimension == 2:
        if not any(isinstance(el, list) for el in RC2_indices):
            RC2_indices=[RC2_indices]
        ...
        #Create lists of point-values
        RCvalue1_list=list(frange(RC1_range[0],RC1_range[1],RC1_range[2]))
        if RCvalue1_list[-1] != RC1_range[1]:
            RCvalue1_list.append(float(RC1_range[1]))    #Adding last specified value to list also
        RCvalue2_list=list(frange(RC2_range[0],RC2_range[1],RC2_range[2]))
        if RCvalue2_list[-1] != RC2_range[1]:
            RCvalue2_list.append(float(RC2_range[1]))    #Adding last specified value to list also
        print("RCvalue1_list: ", RCvalue1_list)
        print("RCvalue2_list: ", RCvalue2_list)
        totalnumpoints=len(RCvalue1_list)*len(RCvalue2_list)
    ...
    def setup_dir(dirname):
        try:
            ...
            print(f"Warning: Could not remove dir {dirname}. Error: {e}")
            
        try:
            os.mkdir(dirname)
        except Exception as e:
            print(f"Error: Could not create dir {dirname}. Error: {e}")
            ashexit()
            
    setup_dir('surface_xyzfiles')
    setup_dir('surface_outfiles')
    setup_dir('surface_mofiles')
...
###########################
    if runmode=='parallel':
            ...
            if dimension == 2:
                ...
                for RCvalue1 in RCvalue1_list:
                    for RCvalue2 in RCvalue2_list:
                        ...
                        if (RCvalue1,RCvalue2) not in surfacedictionary:
                            ...
                            #--- OPTIMIZER LOGIC (UNRELAXED) ---
                            # Running zero-theory with optimizer just to set geometry
                            if optimizer.lower() == 'dlfind':
                                DLFIND_optimizer(fragment=fragment, theory=zerotheory, maxcycle=maxiter, jobtype=dlfind_jobtype,
                                                      constraints=allconstraints, 
                                                      residues=dlfind_residues,
                                                      actatoms=actatoms, # Assumes ActiveRegion=True logic is handled by actatoms
                            ...
                            newfrag = copy.copy(fragment)
                            newfrag.label = (RCvalue1,RCvalue2)
                            xyz_filename = f"RC1_{RCvalue1}-RC2_{RCvalue2}.xyz"
                            newfrag.write_xyzfile(xyzfilename=xyz_filename)
                            shutil.move(xyz_filename, os.path.join("surface_xyzfiles", xyz_filename))
                            surfacepointfragments_lists.append(newfrag)

                print("surfacepointfragments_lists: ", len(surfacepointfragments_lists))
                result_surface = ash.functions.functions_parallel.Job_parallel(fragments=surfacepointfragments_lists, theories=[theory], numcores=numcores)
                ...
                for RCvalue1 in RCvalue1_list:
                    ...
                    if (RCvalue1) not in surfacedictionary:
                        ...
                        # Running zero-theory with optimizer just to set geometry
                        if optimizer.lower() == 'dlfind':
                            DLFIND_optimizerClass(fragment=fragment, theory=zerotheory, maxcycle=maxiter, jobtype=dlfind_jobtype,
                                                  constraints=allconstraints, 
                                                  residues=dlfind_residues,
                                                  actatoms=actatoms,
                                                  charge=charge, mult=mult,
                                                  result_write_to_disk=False, printlevel=0).run()
                        else: # Default to geomeTRIC
                            geomeTRICOptimizer(fragment=fragment, theory=zerotheory, maxiter=maxiter, coordsystem=coordsystem,
                                               constraints=allconstraints, constrainvalue=True, convergence_setting=convergence_setting,conv_criteria=conv_criteria, subfrctor=subfrctor,
                                               ActiveRegion=ActiveRegion, actatoms=actatoms, result_write_to_disk=False)
                        ...
                        newfrag = copy.copy(fragment)
                        newfrag.label = (RCvalue1)
                        xyz_filename = f"RC1_{RCvalue1}.xyz"
                        newfrag.write_xyzfile(xyzfilename=xyz_filename)
                        shutil.move(xyz_filename, os.path.join("surface_xyzfiles", xyz_filename))
                        surfacepointfragments_lists.append(newfrag)
        ...
        #####################
            ...
            # --- PARALLEL RELAXED OPTIMIZER LOGIC ---
            # This mode requires a single optimizer *class* to be passed to Job_parallel.
            # DLFIND_optimizerClass is not compatible as it needs unique constraints at init time.
            # GeomeTRICOptimizerClass is compatible as constraints are passed at run time.
            
            if optimizer.lower() == 'dlfind':
                print("WARNING: 'dlfind' optimizer is not compatible with 'RELAXED' 'parallel' scantype.")
                print("The parallel job runner expects an optimizer class that accepts constraints at runtime.")
                print("Reverting to 'geomeTRIC' for this relaxed parallel scan.")
                optimizer_class_object = GeomeTRICOptimizerClass(maxiter=maxiter, coordsystem=coordsystem,
                                                                 convergence_setting=convergence_setting, conv_criteria=conv_criteria, subfrctor=subfrctor,
                                                                 ActiveRegion=ActiveRegion, actatoms=actatoms)
            else: # Use geomeTRIC
                optimizer_class_object = GeomeTRICOptimizerClass(maxiter=maxiter, coordsystem=coordsystem,
                                                                 convergence_setting=convergence_setting, conv_criteria=conv_criteria, subfrctor=subfrctor,
                                                                 ActiveRegion=ActiveRegion, actatoms=actatoms)
            # --- END PARALLEL RELAXED OPTIMIZER LOGIC ---
                ...
                for RCvalue1 in RCvalue1_list:
                    for RCvalue2 in RCvalue2_list:
                        ...
                        if (RCvalue1,RCvalue2) not in surfacedictionary:
                            ...
                            newfrag = copy.copy(fragment)
                            newfrag.label = (RCvalue1,RCvalue2)
                            #Adding constraints to fragment (for the parallel runner)
                            newfrag.constraints = allconstraints
                            surfacepointfragments_lists.append(newfrag)

                #Parallel opt
                result_surface = ash.functions.functions_parallel.Job_parallel(fragments=surfacepointfragments_lists, theories=[theory], numcores=numcores,
                                                                               Opt=True, optimizer=optimizer_class_object)
                #Moving XYZ-files to surface_xyzfiles
                for RCvalue1 in RCvalue1_list:
                    for RCvalue2 in RCvalue2_list:
                        if (RCvalue1, RCvalue2) in result_surface.worker_dirnames:
                            d = result_surface.worker_dirnames[(RCvalue1,RCvalue2)]
                            shutil.copy(os.path.join(d, "Fragment-optimized.xyz"), os.path.join("surface_xyzfiles", f"RC1_{RCvalue1}-RC2_{RCvalue2}.xyz"))
                        else:
                            print(f"Warning: No worker directory found for point ({RCvalue1}, {RCvalue2})")
            ...
            if dimension == 1:
                ...
                #Moving XYZ-files to surface_xyzfiles
                for RCvalue1 in RCvalue1_list:
                    if (RCvalue1,) in result_surface.worker_dirnames:
                         d = result_surface.worker_dirnames[(RCvalue1,)] # Note: label is a tuple (RCvalue1,)
                         shutil.copy(os.path.join(d, "Fragment-optimized.xyz"), os.path.join("surface_xyzfiles", f"RC1_{RCvalue1}.xyz"))
                    elif (RCvalue1) in result_surface.worker_dirnames: # Check for bare label
                         d = result_surface.worker_dirnames[(RCvalue1)]
                         shutil.copy(os.path.join(d, "Fragment-optimized.xyz"), os.path.join("surface_xyzfiles", f"RC1_{RCvalue1}.xyz"))
                    else:
                        print(f"Warning: No worker directory found for point ({RCvalue1})")
            ...
            if dimension == 2:
                for RCvalue1 in RCvalue1_list:
                    for RCvalue2 in RCvalue2_list:
                        ...
                        print(f"Surfacepoint: {pointcount} / {totalnumpoints}")
                        print(f"RCvalue1: {RCvalue1} RCvalue2: {RCvalue2}")
                        print("Unrelaxed scan. Will use ZeroTheory and optimizer to set geometry.")
                        print("==================================================")
                        pointlabel='RC1_'+str(RCvalue1)+'-'+'RC2_'+str(RCvalue2)
                        if (RCvalue1,RCvalue2) not in surfacedictionary:
                            #Now setting constraints
                            ...
                            # Write geometry to disk
                            xyz_filename = f"RC1_{RCvalue1}-RC2_{RCvalue2}.xyz"
                            fragment.write_xyzfile(xyzfilename=xyz_filename)
                            shutil.move(xyz_filename, os.path.join("surface_xyzfiles", xyz_filename))
                            ...
                            if theory is not None:
                                result = ash.Singlepoint(fragment=fragment, theory=theory, charge=charge, mult=mult)
                                energy = result.energy
                                print(f"RCvalue1: {RCvalue1} RCvalue2: {RCvalue2} Energy: {energy}")
                                if theory.theorytype == "QM":
                                    if keepoutputfiles:
                                        shutil.copyfile(theory.filename+'.out', os.path.join('surface_outfiles', f"{theory.filename}_{pointlabel}.out"))
                                    if keepmofiles:
                                        shutil.copyfile(theory.filename+'.gbw', os.path.join('surface_mofiles', f"{theory.filename}_{pointlabel}.gbw"))
                                else:
                                    print("Warning: For hybrid theories, outputfiles and MO-files are not kept")
                            surfacedictionary[(RCvalue1,RCvalue2)] = energy
                ...
                for RCvalue1 in RCvalue1_list:
                    ...
                    if (RCvalue1) not in surfacedictionary:
                        ...
                        result = ash.Singlepoint(fragment=fragment, theory=theory, charge=charge, mult=mult)
                        energy = result.energy
                        print(f"RCvalue1: {RCvalue1} Energy: {energy}")
                        if theory.theorytype == "QM":
                            if keepoutputfiles:
                                shutil.copyfile(theory.filename+'.out', os.path.join('surface_outfiles', f"{theory.filename}_{pointlabel}.out"))
                            if keepmofiles:
                ...
                for RCvalue1 in RCvalue1_list:
                    for RCvalue2 in RCvalue2_list:
                        ...
                        if (RCvalue1,RCvalue2) not in surfacedictionary:
                            ...
                            #--- OPTIMIZER LOGIC (RELAXED) ---
                            if optimizer.lower() == 'dlfind':
                                # DL-FIND needs to be re-initialized for each point
                                opt = DLFIND_optimizerClass(fragment=fragment, theory=theory, maxcycle=maxiter, jobtype=dlfind_jobtype,
                                                            constraints=allconstraints, 
                                                            residues=dlfind_residues,
                                                            actatoms=actatoms,
                                                            charge=charge, mult=mult,
                                                            result_write_to_disk=False)
                                result = opt.run(charge=charge, mult=mult) # Pass charge/mult to run
                            else: # Default to geomeTRIC
                                result = geomeTRICOptimizer(fragment=fragment, theory=theory, maxiter=maxiter, coordsystem=coordsystem,
                                                            constraints=allconstraints, constrainvalue=True, convergence_setting=convergence_setting, conv_criteria=conv_criteria,
                                                            subfrctor=subfrctor,charge=charge, mult=mult, ActiveRegion=ActiveRegion, actatoms=actatoms, result_write_to_disk=False)
                            ...
                            if theory.theorytype == "QM":
                                if keepoutputfiles:
                                    try:
                                        ...
                                        print(f"Warning: could not copy output file. {e}")
                                if keepmofiles:
                                    try:
                                        shutil.copyfile(theory.filename+'.gbw', os.path.join('surface_mofiles', f"{theory.filename}_{pointlabel}.gbw"))
                                    except Exception as e:
                                        print(f"Warning: could not copy MO file. {e}")
                            else:
                                print("Warning: For hybrid theories, outputfiles and MO-files are not kept")
                            surfacedictionary[(RCvalue1,RCvalue2)] = energy
                    ...
                    if (RCvalue1) not in surfacedictionary:
                        ...
                        if optimizer.lower() == 'dlfind':
                            opt = DLFIND_optimizerClass(fragment=fragment, theory=theory, maxcycle=maxiter, jobtype=dlfind_jobtype,
                                                        ...
                                                        actatoms=actatoms,
                                                        result_write_to_disk=False)
                            result = opt.run(charge=charge, mult=mult)
                        else: # Default to geomeTRIC
                            result = geomeTRICOptimizer(fragment=fragment, theory=theory, maxiter=maxiter, coordsystem=coordsystem,
                                                        constraints=allconstraints, constrainvalue=True, convergence_setting=convergence_setting, conv_criteria=conv_criteria,
                                                        subfrctor=subfrctor,charge=charge, mult=mult,
